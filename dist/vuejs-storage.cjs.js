...
function install(Vue) {
    Vue.mixin({
        created: function () {
            var _this = this;
            function cloneOptions(options, ref) {
                var clonedOptions = {
                    keys: [],
                    namespace: ''
                };
            ...
            if ('storage' in this.$options) {
                var options = this.$options.storage;
                if (Array.isArray(options))
                    options.forEach(function (opt) { return applyPersistence(_this, cloneOptions(opt, _this)); });
                else
                    applyPersistence(this, cloneOptions(options, this));
            }
        }
    });
}
